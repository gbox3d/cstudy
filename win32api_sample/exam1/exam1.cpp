...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   CreateWindow(L"static", L"몸무게를 입력하세요.", WS_CHILD | WS_VISIBLE,
	   0, 200, 180, 25, hWnd, (HMENU)-1, hInst, NULL);
    ...
    case WM_COMMAND:
        {
			...
			{
				TCHAR szTemp[256];
				GetWindowText(GetDlgItem(hWnd, 3001), szTemp, sizeof(szTemp));

				TCHAR szName[256];
				lstrcpyW(szName, szTemp);
				
				int nAge;
				GetWindowText(GetDlgItem(hWnd, 3002), szTemp, sizeof(szTemp));
				nAge = _wtoi(szTemp);

				double fWeight;
				GetWindowText(GetDlgItem(hWnd, 3003), szTemp, sizeof(szTemp));
				fWeight = _wtof(szTemp);

				HDC hdc = GetDC(hWnd);
				TCHAR szBuf[256];
				swprintf_s(szBuf, 256, L"당신의 이름은 %s 입니다. 나이는 %d살입니다. %lfkg", szName, nAge,fWeight);
